...
select book_title , book_price, book_price + 10000 as '더하기' from book;
...
select cart_no ,buyer from cart;
# [문제 4] 각 도서의 가격을 10% 인상한 금액 출력
select book_title,book_price as '원래금액',cast(book_price*1.1 as DECIMAL)  as '인상후금액' from book;
# [문제 5] 각 도서의 가격을 10% 인하한 금액 출력
select book_title,book_price as '원래금액',book_price*0.9 as '인하후금액' from book;
...
select member_name, member_pw from member where member_id like '%qwer';
# [문제 2] book테이블에서 book_kind가 문학(100)인 책정보 가져오기
select * from book where book_kind like '100';
...
select * from book where book_publishing_date < '2020-01-01';
# [문제 6] book테이블에서 컴퓨터(300)이면서 가격이 15,000원 이상인 책정보 가져오기
select * from book where book_kind like '300' and book_price >= 15000;
# [문제 7] book테이블에서 재고가 없는 책정보 가져오기
select * from book where book_count <= 0;
# [문제 8] book테이블에서 문학(100)이거나 컴퓨터(300)인 책정보 가져오기
select * from book where book_kind like '300' or book_kind like '100';
...
select * from member where length(member_name) =12;

# 정렬 order by : 순서 by 뒤에 기준으로 순서대로 정렬해서 view table 출력
# 1 -> select 
# 2 -> from 테이블명
# 3 -> where 조건절 
# 4 -> order by 정렬조건 . 순서 중요하다 !!
# asc -> 오름차순 정렬 desc -> 내림차순 정렬

select book_title, book_publishing_date from book; -- 데이터를 추가 한 순으로 나온다 .
select book_title, book_publishing_date from book order by book_publishing_date desc;

# [문제] book테이블에서 도서명에 '자바'가 들어가는 도서 가져오기
# [조건] 도서명을 기준으로 정렬

select book_title from book where book_title like '%자바%' order by book_title asc;
